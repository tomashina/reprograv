...
class ControllerExtensionModuleMpphoto extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link($this->extension_path . 'module/mpphoto', $this->token.'=' . $this->session->data[$this->token] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link($this->extension_path . 'module/mpphoto', $this->token.'=' . $this->session->data[$this->token], true);
		} else {
			$data['action'] = $this->url->link($this->extension_path . 'module/mpphoto', $this->token.'=' . $this->session->data[$this->token] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['mpgallery_id'])) {
			$data['mpgallery_id'] = $this->request->post['mpgallery_id'];
		} elseif (isset($module_info['mpgallery_id'])) {
			$data['mpgallery_id'] = $module_info['mpgallery_id'];
		} else {
			$data['mpgallery_id'] = 0;
		}
		...
		if (isset($this->request->post['photo_name'])) {
			...
			$gallery_info = $this->{'model_' . $this->extension_model . 'mpphotogallery_mpgallery'}->getMpGallery($module_info['mpgallery_id']);
		...
		} elseif (isset($module_info['photo_description'])) {
		...
		$this->response->setOutput($this->viewLoad($this->extension_path . 'module/mpphoto', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', $this->extension_path . 'module/mpphoto')) {
