...
class ControllerExtensionModuleClientTranslateExpert extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->post['module_client_translate_expert_char_count']))
			{
				$this->request->post['module_client_translate_expert_char_count'] = 0;
			}
			if (!isset($this->request->post['module_client_translate_expert_reset_char_count_on_start_month']))
			{
				$this->request->post['module_client_translate_expert_reset_char_count_on_start_month'] = 0;
			}
			if (!isset($this->request->post['module_client_translate_expert_show_char_count_globally']))
			{
				$this->request->post['module_client_translate_expert_show_char_count_globally'] = 0;
			}
			if (!isset($this->request->post['module_client_translate_expert_last_reset_char_month']))
			{
				$this->request->post['module_client_translate_expert_last_reset_char_month'] = date('m');
			}
		...
		if (isset($this->request->post['module_client_translate_expert_show_char_count_globally'])) {
			$data['module_client_translate_expert_show_char_count_globally'] = $this->request->post['module_client_translate_expert_show_char_count_globally'];
		} else {
			$data['module_client_translate_expert_show_char_count_globally'] = $this->config->get('module_client_translate_expert_show_char_count_globally');
		}

		if (isset($this->request->post['module_client_translate_expert_reset_char_count_on_start_month'])) {
			$data['module_client_translate_expert_reset_char_count_on_start_month'] = $this->request->post['module_client_translate_expert_reset_char_count_on_start_month'];
		} else {
			$data['module_client_translate_expert_reset_char_count_on_start_month'] = $this->config->get('module_client_translate_expert_reset_char_count_on_start_month');
		}
		
		$data['module_client_translate_expert_char_count'] = number_format($this->config->get('module_client_translate_expert_char_count'), 0, '.', '`');
	...
	public function translatedCharCountInfo()
	{
		...
		$charCount = $this->config->get("module_client_translate_expert_char_count");
	...
	public function resetCharCount()
	{
		$currentMonth = date('m');
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSettingValue('module_client_translate_expert', 'module_client_translate_expert_char_count', 0);
		$this->model_setting_setting->editSettingValue('module_client_translate_expert', 'module_client_translate_expert_last_reset_char_month', $currentMonth);
	...
	private function storeTranslatedCharCount($charCount = 0)
	{
		...
		$prevCharCount = $this->config->get("module_client_translate_expert_char_count");
		$newCharCount = $charCount + ($prevCharCount ? $prevCharCount : 0);

		$this->load->model('setting/setting');
		$this->model_setting_setting->editSettingValue('module_client_translate_expert', 'module_client_translate_expert_char_count', $newCharCount);
	}
	
	private function resetCharCountOnStartMonth()
	{
		if ($this->config->get("module_client_translate_expert_reset_char_count_on_start_month"))
		{
			$lastMonth = $this->config->get("module_client_translate_expert_last_reset_char_month");
			$currentMonth = date('m');
