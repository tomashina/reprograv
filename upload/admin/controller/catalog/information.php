...
class ControllerCatalogInformation extends Controller {
    ...
    if (isset($this->request->post['information_pdfs'])) {
        ...
        // Ako ništa nije poslano, počisti postojeće
        $this->load->model('catalog/information_pdf');
        $this->model_catalog_information_pdf->setPdfs((int)$this->request->get['information_id'], []);
    }
	...
	protected function getForm() {
		...
		if (isset($this->request->post['information_pdfs'])) {
		    $data['information_pdfs'] = $this->request->post['information_pdfs'];
		} elseif (!empty($this->request->get['information_id'])) {
		    $data['information_pdfs'] = $this->model_catalog_information_pdf->getPdfs((int)$this->request->get['information_id']);
		} else {
		    $data['information_pdfs'] = array();
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['information_description'] as $language_id => $value) {
			if ((utf8_strlen($value['title']) < 1) || (utf8_strlen($value['title']) > 256)) {
