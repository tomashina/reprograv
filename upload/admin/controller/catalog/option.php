...
class ControllerCatalogOption extends Controller {
	...
	protected function getForm() {
        ...
        foreach ($option_values as $key => $value) {
            $query = $this->db->query("SELECT color FROM " . DB_PREFIX . "option_value WHERE option_value_id = '" . (int)$value['option_value_id'] . "'");
            $option_values[$key]['color'] = $query->row ? $query->row['color'] : '';
        }
