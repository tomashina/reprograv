...
class ControllerCatalogProduct extends Controller {
	...
	public function importTmpImages() {
    $this->load->language('catalog/product');
    $this->response->addHeader('Content-Type: application/json');
    ...
    if (!$this->user->hasPermission('modify', 'catalog/product')) {
        $json['error'] = 'Nemate dopuštenje za izmjene proizvoda.';
        return $this->response->setOutput(json_encode($json));
    }

    $product_id = isset($this->request->get['product_id']) ? (int)$this->request->get['product_id'] : 0;
    if (!$product_id) {
        $json['error'] = 'Nedostaje product_id.';
        return $this->response->setOutput(json_encode($json));
    }

    // 1) Učitaj naziv proizvoda (po default jeziku)
    $language_id = (int)$this->config->get('config_language_id');
    $q = $this->db->query("SELECT name FROM " . DB_PREFIX . "product_description WHERE product_id = " . (int)$product_id . " AND language_id = " . $language_id . " LIMIT 1");
    if (!$q->num_rows) {
        $json['error'] = 'Ne mogu pronaći naziv proizvoda.';
        return $this->response->setOutput(json_encode($json));
    }
    $product_name = $q->row['name'];

    // 2) Dohvati sve kandidate iz tmp_sliketrodat
    $tmp = $this->db->query("SELECT `Naziv`, `Slike` FROM tmp_sliketrodat");
    ...
    $splitImages = function($s) {
        $out = [];
        foreach (preg_split('/[|,;\\s]+/u', (string)$s) as $p) {
            $p = trim($p);
            if ($p) $out[] = $p;
        }
    ...
    $target_name = $normalize($product_name);

    // 3) Pronađi red(ove) gdje Naziv (bez HTML) == target_name
    $images_all = [];
    foreach ($tmp->rows as $row) {
        if ($normalize($row['Naziv']) === $target_name) {
            $images_all = array_merge($images_all, $splitImages($row['Slike']));
        }
    }
    $images_all = array_values(array_unique($images_all));

    if (!$images_all) {
        $json['error'] = 'Nema slika u tmp_sliketrodat za naziv: ' . $product_name;
        return $this->response->setOutput(json_encode($json));
    }

    // 4) Provjeri postoji li već glavna slika
    $has_main = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$product_id . " LIMIT 1")->row['image'];
    $added = 0;
    $set_main = false;

    // 5) Priprema direktorija za download (ako URL)
    $import_dir = DIR_IMAGE . 'catalog/import/';
    if (!is_dir($import_dir)) {
        @mkdir($import_dir, 0777, true);
    }

    $makeLocalIfUrl = function($url) use ($import_dir) {
        // Ako je URL, skinuti ga; inače tretirati kao relativni put unutar image/
        if (preg_match('~^https?://~i', $url)) {
            $basename = basename(parse_url($url, PHP_URL_PATH)) ?: (uniqid('img_') . '.jpg');
            // izbjegni duplikate
            $dest_rel = 'catalog/import/' . $basename;
            $dest_abs = $import_dir . $basename;

            // ako već postoji, dodaj suffix
            if (file_exists($dest_abs)) {
                $name = pathinfo($basename, PATHINFO_FILENAME);
                $ext  = pathinfo($basename, PATHINFO_EXTENSION);
                $basename = $name . '_' . substr(md5($url), 0, 8) . ($ext ? ('.' . $ext) : '');
                $dest_rel = 'catalog/import/' . $basename;
                $dest_abs = $import_dir . $basename;
            }

            // grubi downloader (cURL ako postoji, fallback na file_get_contents)
            $data = '';
            ...
            if ($data) {
                @file_put_contents($dest_abs, $data);
                return $dest_rel; // relativno u odnosu na DIR_IMAGE
            } else {
                // ako se ne može skinuti, upiši izvorni URL (možda tema podržava eksterne)
                return $url;
            }
        } else {
            // lokalno: normaliziraj bez vodećeg slash-a
            return ltrim($url, '/');
        }
    };

    // 6) Umetni u product_image (i postavi glavnu ako treba)
    $sort = 0;
    $escaped_images = [];

    foreach ($images_all as $img) {
        $path = $makeLocalIfUrl($img);
        $escaped = $this->db->escape($path);
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = " . (int)$product_id . ", image = '" . $escaped . "', sort_order = " . (int)$sort);
        $added++;
        $escaped_images[] = $escaped;
        $sort++;
    }

    // Postavi glavnu ako ne postoji i nešto smo dodali
    if (!$has_main && $added > 0) {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $escaped_images[0] . "' WHERE product_id = " . (int)$product_id);
        $set_main = true;
    }

    $json['added'] = $added;
    $json['set_main'] = $set_main;
