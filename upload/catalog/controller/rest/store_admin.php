...
class ControllerRestStoreAdmin extends RestAdminController
{
    ...
    public function stats()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $sale_total = $this->model_rest_restadmin->getTotalSales();

            $totals['sale_total'] = (float)round($sale_total);

            $order_total = $this->model_rest_restadmin->getTotalOrders();

            $totals['order_total'] = (int)$order_total;

            $customer_total = $this->model_rest_restadmin->getTotalCustomers();

            $totals['customer_total'] = (int)$customer_total;

            $totals['product_total'] = (int)$this->model_rest_restadmin->getTotalProducts();

            $this->json['data'] = $totals;
