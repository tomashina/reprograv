    ...
    */
    public function productclasses()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if(empty($type) || $type == "return_statuses") {
                $this->json['data']['return_statuses'] = $this->model_rest_restadmin->getReturnStatuses();
            }

            if(empty($type) || $type == "return_reasons") {
                $this->json['data']['return_reasons'] = $this->model_rest_restadmin->getReturnReasons();
            }

            if(empty($type) || $type == "return_actions") {
                $this->json['data']['return_actions'] = $this->model_rest_restadmin->getReturnActions();
            }
    ...
    */
    ...
    private function getCountryInfo($country_info, $addZone = true)
    {
        ...
        $info = array(
            ...
            'postcode_required' => (int)$country_info['postcode_required'],
            'status' => (int)$country_info['status']
        );
        if ($addZone) {
